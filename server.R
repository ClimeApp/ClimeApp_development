...
server <- function(input, output, session) {
  ...
  observeEvent(input$ts_dblclick1,{
    if (input$custom_features_ts == TRUE & input$feature_ts == "Line"){
      ...
      observeEvent(input$update_metadata_ts, {
        ...
        if (!is.null(first_sheet_name) &&
            first_sheet_name == "custom_map_ts") {
      ...
      output$download_map_data        <- downloadHandler(
        ...
        content = function(file) {
          if (input$file_type_map_data == "csv") {
            ...
            write.xlsx(
              rewrite_maptable(map_data(), subset_lons_primary(), subset_lats_primary()),
              file,
              row.names = FALSE,
              col.names = FALSE
            )
 ...
 # Select variable timeseries data
  ...
  ### Regression plots ----
  output$download_map2 <- downloadHandler(
    filename = function() {
      paste(plot_titles_2()$file_title,
            "-map.",
            input$file_type_map2,
            sep = "")
    },
    content = function(file) {
      if (input$file_type_map2 == "png") {
        png(
          file,
          width = map_dimensions_2()[3],
          height = map_dimensions_2()[4],
          res = 200,
          bg = "transparent"
        )
      } else if (input$file_type_map2 == "jpeg") {
        ...
        pdf(
          file,
          width = map_dimensions_2()[3] / 200,
          height = map_dimensions_2()[4] / 200,
          bg = "transparent"
        )
      }
      print(map_plot_2())
  ...
  output$download_map_reg_res       <- downloadHandler(filename = function() {paste(plot_titles_reg_res()$file_title, "-map.", input$plot_type_reg_res, sep = "")},
                                               content = function(file) {
                                                 if (input$plot_type_reg_res == "png") {
                                                   png(file, width = map_dimensions_reg()[3], height = map_dimensions_reg()[4], res = 200, bg = "transparent")
                                                 } else if (input$plot_type_reg_res == "jpeg") {
                                                   jpeg(file, width = map_dimensions_reg()[3], height = map_dimensions_reg()[4], res = 200, bg = "white")
                                                 } else {
                                                   pdf(file, width = map_dimensions_reg()[3] / 200, height = map_dimensions_reg()[4] / 200, bg = "transparent")
                                                 }
                                                 print(ref_map_plot())  # Use print to ensure the plot is fully rendered
                                                 dev.off()}
  )
  ...
  # Download Regression Maps
  # Coefficients
  output$download_reg_coe_plot <- downloadHandler(
    filename = function() {
      paste0(plot_titles_reg_coeff()$file_title, "-map.", input$reg_coe_plot_type)
    },
    content = function(file) {
      width <- plot_dimensions_reg()[3]
      height <- plot_dimensions_reg()[4]
      
      if (input$reg_coe_plot_type == "png") {
        png(file, width = width, height = height, res = 200, bg = "transparent")
        
      } else if (input$reg_coe_plot_type == "jpeg") {
        jpeg(file, width = width, height = height, res = 200, bg = "white")
        
      } else {
        pdf(file, width = width / 200, height = height / 200, bg = "transparent")
      }
      print(reg_coef_map())
      dev.off()
    }
  ...
  # P-Values
  output$download_reg_pval_plot <- downloadHandler(
    filename = function() {
      paste0(plot_titles_reg_pval()$file_title, "-map.", input$reg_pval_plot_type)
    },
    content = function(file) {
      width <- plot_dimensions_reg()[3]
      height <- plot_dimensions_reg()[4]
  ...
  # Residuals
  output$download_reg_res_plot <- downloadHandler(
    filename = function() {
      paste0(plot_titles_reg_res()$file_title, "-map.", input$reg_res_plot_type)
    },
    content = function(file) {
      width <- plot_dimensions_reg()[3]
      height <- plot_dimensions_reg()[4]
  ...
  # Download Regression Map Data
  # Coefficients
  output$download_reg_coe_plot_data <- downloadHandler(
    filename = function() {
      paste0(plot_titles_reg_coeff()$file_title, "-mapdata.", input$reg_coe_plot_data_type, sep = "")
    },
    content  = function(file) {
      if (input$reg_coe_plot_data_type == "csv") {
        ...
        write.xlsx(
          rewrite_maptable(reg_coef_table(), NA, NA),
          file,
          row.names = FALSE,
          col.names = FALSE
        )
  ...
  #   filename = function() {
  #     paste(
  #       plot_titles_reg()$Download_title,
  #       "-mapdata.",
